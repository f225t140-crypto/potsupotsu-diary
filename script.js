...
const LEGACY_DEFAULT_PROFILE_ICON_ROWS = [
  ".........", "...ccc...", "..ccccc..", ".ccscscc.", ".ccccccc.",
  ".cceeecc.", "..ccccc..", "...ggg...", "........."
];

const LEGACY_DEFAULT_PROFILE_ICON_PALETTE = {
  c: "#a95747",
  s: "#fffdf9",
  e: "#1d1d1f",
  g: "#708266"
...
const iconEditorCloseButton = document.querySelector("#icon-editor-close-button");
...
const iconDrawingPanel = document.querySelector("#icon-drawing-panel");
...
const headerBackLabel = document.querySelector("#header-back-label");
const headerScreenTitle = document.querySelector("#header-screen-title");
const profileAvatarGrid = document.querySelector("#profile-avatar-grid");
const profileDiaryGrid = document.querySelector("#profile-diary-grid");
const profileDiaryEmpty = document.querySelector("#profile-diary-empty");
const partnerProfileAvatarGrid = document.querySelector("#partner-profile-avatar-grid");
const partnerProfileDiaryGrid = document.querySelector("#partner-profile-diary-grid");
const partnerProfileBlockButton = document.querySelector("#partner-profile-block-button");
const partnerProfileSafetyMenu = document.querySelector("#partner-profile-safety-menu");
const partnerProfileMenuToggle = document.querySelector("#partner-profile-menu-toggle");
...
const drawTutorialTipProgress = document.querySelector("#draw-tutorial-tip-progress");
const drawTutorialTipNext = document.querySelector("#draw-tutorial-tip-next");
...
let profileReturnScreen = "today";
let partnerProfileReturnScreen = "friends";
...
const DRAW_TUTORIAL_STEPS = [
  {
    copy: "まず、パレットから使いたい色を選びます。",
    target: "#drawing-panel"
  },
  {
    copy: "キャンバスのマスをタップすると、選んだ色を置けます。",
    target: "#editor-grid"
  },
  {
    copy: "「消す」を選ぶと、タップしたマスの色を消せます。",
    target: "#eraser-button"
  },
  {
    copy: "「スポイト」を選ぶと、絵の中から色を選べます。",
    target: "#eyedropper-button"
  },
  {
    copy: "「塗る」を選ぶと、つながった同じ色の範囲をまとめて塗れます。",
    target: "#bucket-button"
  },
  {
    copy: "「戻す」と「進む」で、直前の操作をやり直せます。",
    target: "#undo-button, #redo-button"
  },
  {
    copy: "描けたら「タグを付ける」へ進みます。タグは選ばなくても保存できます。",
    target: "#go-tags-button"
  }
];
...
function normalizeState(saved) {
  ...
  const savedProfileIcon = normalizePixels(saved.profileIconPixels);
  const legacyDefaultProfileIcon = rowsToPixels(
    LEGACY_DEFAULT_PROFILE_ICON_ROWS,
    LEGACY_DEFAULT_PROFILE_ICON_PALETTE
  );
  const usesLegacyDefaultIcon = savedProfileIcon
    && savedProfileIcon.every((color, index) => color === legacyDefaultProfileIcon[index]);
  const normalized = {
    version: 2,
    ...
    profileIconPixels: savedProfileIcon && !usesLegacyDefaultIcon
      ? savedProfileIcon
      : initial.profileIconPixels
  };
...
function hideTutorialTips() {
  ...
  document.querySelectorAll(".is-draw-tutorial-target").forEach((element) => {
    element.classList.remove("is-draw-tutorial-target");
  });
...
function restoreIconEditor() {
  ...
  document.body.classList.remove("is-icon-editor-sheet-open");
}
...
function startTutorial() {
  ...
  drawTutorialGuideIndex = 0;
  hideTutorialTips();
  showScreen("tutorial");
}

function showDrawTutorialTip(stepIndex = drawTutorialGuideIndex) {
  drawTutorialGuideIndex = Math.max(0, Math.min(DRAW_TUTORIAL_STEPS.length - 1, stepIndex));
  const step = DRAW_TUTORIAL_STEPS[drawTutorialGuideIndex];
  document.querySelectorAll(".is-draw-tutorial-target").forEach((element) => {
    element.classList.remove("is-draw-tutorial-target");
  });
  document.querySelectorAll(step.target).forEach((element) => {
    element.classList.add("is-draw-tutorial-target");
  });
  drawTutorialTipCopy.textContent = step.copy;
  drawTutorialTipProgress.textContent = `${drawTutorialGuideIndex + 1} / ${DRAW_TUTORIAL_STEPS.length}`;
  drawTutorialTipNext.textContent = drawTutorialGuideIndex === DRAW_TUTORIAL_STEPS.length - 1
    ? "閉じる"
    : "次へ";
  drawTutorialTip.hidden = false;
}

function closeDrawTutorialTip() {
  drawTutorialTip.hidden = true;
  document.querySelectorAll(".is-draw-tutorial-target").forEach((element) => {
    element.classList.remove("is-draw-tutorial-target");
  });
...
function advanceDrawTutorialTip() {
  if (drawTutorialGuideIndex >= DRAW_TUTORIAL_STEPS.length - 1) {
    closeDrawTutorialTip();
    return;
  }
  showDrawTutorialTip(drawTutorialGuideIndex + 1);
}
...
function renderIconPalette() {
  ...
  renderPaletteTabSet(iconPaletteTabs, (paletteId) => setActivePalette(paletteId, "icon"));
...
function renderProfileTagSettings() {
  ...
  state.profileTags.forEach((tag) => {
    ...
    button.insertAdjacentHTML("beforeend", '<svg class="sf-icon profile-tag-remove-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="m7 7 10 10"></path><path d="M17 7 7 17"></path></svg>');
...
function setIconEditorOpen(isOpen) {
  ...
  document.body.classList.toggle("is-icon-editor-sheet-open", isOpen && currentScreen === "settings");
  if (isOpen) {
    isIconPalettePanelOpen = false;
    renderIconEditor();
    updateIconPalettePanelState();
...
function renderSettings() {
  ...
  settingsBackButton.querySelector("span").textContent = settingsReturnScreen === "profile"
    ? "プロフィール"
    : "ホーム";
...
function animateIconCell(index, className, delayMs = 0, properties = {}, options = {}) {
  animateGridCell(iconEditorGrid, index, className, delayMs, properties, options);
}
...
function updateIconPaletteBuilder() {
  ...
  iconDrawingPanel.classList.toggle("is-custom-palette", isCustomPalette);
...
function paintIconIndex(index) {
  ...
  const previousColor = iconPixels[index];
  iconPixels[index] = nextColor;
  const column = index % 9;
  const row = Math.floor(index / 9);
  const eraseDirection = column % 2 === 0 ? -1 : 1;
  animateIconCell(index, isIconErasing ? "is-erased" : "is-painted", 0, {
    "--effect-color": isIconErasing ? previousColor : nextColor,
    ...
    toColor: nextColor,
    durationMs: isIconErasing ? 360 : 300
  });
  renderIconDraftPreview();
  return true;
}
...
function floodFillIcon(index) {
  ...
  const originColumn = index % 9;
  filledIndices.forEach((filledIndex) => {
    const row = Math.floor(filledIndex / 9);
    const column = filledIndex % 9;
    ...
    animateIconCell(filledIndex, "is-fill-wave", distance * 24, {
      "--effect-color": nextColor,
      "--wave-x": `${waveColumn * 9}px`,
      "--wave-y": `${waveRow * 9}px`,
      "--wave-mid-x": `${waveColumn * 4}px`,
      "--wave-mid-y": `${waveRow * 4}px`
    }, {
      toColor: nextColor,
      durationMs: 500
    });
  });
  pulseToolButton(iconBucketButton);
...
function useIconEyedropper(index) {
  ...
  isIconEyedropper = false;
  isIconBucketFill = false;
  animateIconCell(index, "is-sampled", 0, {
    "--effect-color": sampledColor
  }, {
    toColor: sampledColor,
    durationMs: 520
  });
  renderIconPalette();
  updateIconToolState();
...
function undoIcon() {
  ...
  renderIconEditor();
  playIconHistoryAnimation(before, iconPixels, "undo");
  updateIconToolState();
}
...
function redoIcon() {
  ...
  renderIconEditor();
  playIconHistoryAnimation(before, iconPixels, "redo");
  updateIconToolState();
}

function playIconHistoryAnimation(before, after, direction) {
  const changed = changedPixelIndices(before, after);
  pulseToolButton(direction === "undo" ? iconUndoButton : iconRedoButton);
  ...
  changed.forEach((index) => {
    ...
    animateIconCell(index, className, diagonal * 8, {
...
function clearProfileIconDraft() {
  ...
  resetIconClearConfirmation();
  playIconClearDropAnimation(before);
}

function playIconClearDropAnimation(before) {
  pulseToolButton(iconClearButton);
  renderIconDraftPreview();
  ...
  iconEditorGrid.classList.add("is-clearing");
  ...
  before.forEach((color, index) => {
    ...
    animateIconCell(index, "is-clear-drop", delay, {
      "--drop-x": `${(column - 4) * 5}px`,
      "--fall-distance": `${112 + row * 9}px`,
      "--cell-tilt": `${(column % 2 === 0 ? -1 : 1) * (45 + row * 4)}deg`
    }, {
      durationMs: 580
    });
  });
  ...
  setTimeout(() => {
    renderIconEditor();
    iconEditorGrid.classList.remove("is-clearing");
...
function renderPaletteTabs() {
  renderPaletteTabSet(paletteTabs, (paletteId) => setActivePalette(paletteId, "draw"));
}
...
function updatePaletteBuilder() {
  ...
  drawingPanel.classList.toggle("is-custom-palette", isCustomPalette);
...
function updatePalettePanelState() {
  ...
  paletteToggleButton.textContent = isPalettePanelOpen ? "閉じる" : "パレット";
  paletteToggleButton.setAttribute("aria-expanded", String(isPalettePanelOpen));
}

function updateIconPalettePanelState() {
  iconDrawingPanel.classList.toggle("is-collapsed", !isIconPalettePanelOpen);
  iconPalettePanelBody.hidden = !isIconPalettePanelOpen;
  iconPaletteToggleButton.textContent = isIconPalettePanelOpen ? "閉じる" : "パレット";
...
function setActivePalette(paletteId, source = "draw") {
  ...
  renderIconPalette();
  if (paletteId !== "custom") {
    if (source === "draw") isPalettePanelOpen = false;
    if (source === "icon") isIconPalettePanelOpen = false;
  }
  updatePalettePanelState();
  updateIconPalettePanelState();
...
function animateGridCell(grid, index, className, delayMs = 0, properties = {}, options = {}) {
  ...
  setTimeout(() => {
    if (grid.children[index] !== cell || cell.dataset.animationToken !== token) return;
    resetCellAnimation(cell);
  }, delay + duration + 90);
}

function animateCell(index, className, delayMs = 0, properties = {}, options = {}) {
  animateGridCell(editorGrid, index, className, delayMs, properties, options);
}
...
function renderRecords() {
  ...
  const currentMonthButton = document.querySelector("#record-current-month");
  currentMonthButton.hidden = selectedMonthKey === monthKey(today);
  currentMonthButton.parentElement.hidden = currentMonthButton.hidden;
...
function renderFriends() {
  ...
  friends.forEach((sample) => {
    ...
    profileButton.className = "friend-profile-button";
    profileButton.type = "button";
    profileButton.setAttribute("aria-label", `${sample.name}さんのプロフィールを開く`);
    profileButton.addEventListener("click", () => openPartnerProfile(sample));
    const avatarFrame = document.createElement("span");
    avatarFrame.className = "friend-avatar-frame";
    const avatar = document.createElement("span");
    avatar.className = "pixel-grid pixel-preview friend-avatar-pixels";
    avatar.setAttribute("aria-hidden", "true");
    makeGrid(avatar, getSampleProfileIcon(sample));
    ...
    chevron.className = "friend-row-chevron";
    chevron.innerHTML = '<svg class="sf-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="m9 6 6 6-6 6"></path></svg>';
    profileButton.append(avatarFrame, identity, chevron);
    ...
    menuToggle.innerHTML = '<svg class="sf-icon sf-icon--more" viewBox="0 0 24 24" aria-hidden="true"><circle cx="5" cy="12" r="1.5"></circle><circle cx="12" cy="12" r="1.5"></circle><circle cx="19" cy="12" r="1.5"></circle></svg>';
    ...
    top.append(profileButton, partnerActions);
    ...
    worksCount.textContent = `${works.length}件`;
    worksHeading.append(worksTitle, worksCount);
    const strip = document.createElement("div");
    strip.className = "friend-work-strip";
    strip.tabIndex = 0;
    strip.setAttribute("aria-label", `${sample.name}さんから届いた絵日記`);
    ...
    works.forEach((work, workIndex) => {
      ...
      const weekday = workDate
        ? new Intl.DateTimeFormat("ja-JP", { weekday: "short" }).format(workDate)
        : "";
      label.textContent = workDate
        ? `${workDate.getMonth() + 1}/${workDate.getDate()}（${weekday}）`
        : work.date;
      ...
      renderTagRow(diaryTags, workTags.slice(0, 2));
      if (workTags.length > 2) {
        const moreTags = document.createElement("span");
        moreTags.className = "friend-work-tag-more";
        moreTags.textContent = `+${workTags.length - 2}`;
        diaryTags.appendChild(moreTags);
      }
...
function getSampleProfileIcon(sample) {
  return rowsToPixels(DEFAULT_PROFILE_ICON_ROWS, { p: sample.avatar });
}

function createProfileDiaryCard({ pixels: diaryPixels, date, tags = [], label, isLatest = false, onOpen }) {
  const button = document.createElement("button");
  button.className = "profile-diary-card";
  ...
  const preview = document.createElement("span");
  preview.className = "pixel-grid pixel-preview profile-diary-preview";
  preview.setAttribute("aria-hidden", "true");
  makeGrid(preview, diaryPixels);

  const meta = document.createElement("span");
  meta.className = "profile-diary-meta";
  const time = document.createElement("time");
  time.dateTime = date;
  const diaryDate = dateFromKey(date);
  const weekday = diaryDate
    ? new Intl.DateTimeFormat("ja-JP", { weekday: "short" }).format(diaryDate)
    : "";
  time.textContent = diaryDate
    ? `${diaryDate.getMonth() + 1}/${diaryDate.getDate()}（${weekday}）`
    : date;
  meta.appendChild(time);

  const tagRow = document.createElement("span");
  tagRow.className = "profile-diary-tags";
  tags.slice(0, 2).forEach((tag) => tagRow.appendChild(makeTagChip(tag)));
  if (tags.length > 2) {
    const more = document.createElement("span");
    more.className = "profile-diary-tag-more";
    more.textContent = `+${tags.length - 2}`;
    tagRow.appendChild(more);
  }

  button.append(preview, meta, tagRow);
  return button;
}

function renderSelfProfile() {
  const records = [...state.records].sort((a, b) => b.date.localeCompare(a.date));
  const activePartners = state.partnerIds.filter((id) => !state.blockedIds.includes(id));

  makeGrid(profileAvatarGrid, state.profileIconPixels);
  document.querySelector("#profile-display-name").textContent = state.profileName;
  renderTagRow(document.querySelector("#profile-display-tags"), state.profileTags, "profile-chip");
  document.querySelector("#profile-record-count").textContent = records.length;
  document.querySelector("#profile-partner-count").textContent = activePartners.length;
  profileDiaryGrid.innerHTML = "";
  profileDiaryEmpty.hidden = records.length > 0;

  records.forEach((record) => {
    profileDiaryGrid.appendChild(createProfileDiaryCard({
      pixels: record.pixels,
      date: record.date,
      tags: record.tags || [],
      label: `${formatRecordDate(record.date)}の自分の絵日記を開く`,
      onOpen: () => openRecordDetail(record, "profile")
    }));
  });
}

function renderPartnerProfile() {
  const sample = activePartnerProfile;
  if (!sample) {
    showScreen("friends");
    return;
  }
  ...
  const works = getSampleWorks(sample);
  makeGrid(partnerProfileAvatarGrid, getSampleProfileIcon(sample));
  document.querySelector("#partner-profile-name").textContent = sample.name;
  renderTagRow(document.querySelector("#partner-profile-tags"), sample.profileTags, "profile-chip");
  document.querySelector("#partner-profile-work-count").textContent = works.length;
  partnerProfileBlockButton.setAttribute("aria-label", `${sample.name}さんをブロックする`);
  partnerProfileSafetyMenu.open = false;
  partnerProfileMenuToggle.setAttribute("aria-label", `${sample.name}さんの操作を開く`);
  partnerProfileDiaryGrid.innerHTML = "";

  works.forEach((work, index) => {
    partnerProfileDiaryGrid.appendChild(createProfileDiaryCard({
      pixels: work.pixels,
      date: work.date,
      tags: work.tags || [],
      label: `${sample.name}さんの${formatRecordDate(work.date)}の絵日記を開く`,
      isLatest: index === 0,
      onOpen: () => openFriendDetail(sample, work, "partner-profile")
    }));
  });
}

function openPartnerProfile(sample, returnScreen = "friends") {
  activePartnerProfile = sample;
  partnerProfileReturnScreen = returnScreen;
  showScreen("partner-profile");
}

function openRecordDetail(record, returnScreen = "records") {
...
function openFriendDetail(sample, work = getSampleWorks(sample)[0], returnScreen = "friends") {
  detailReturnScreen = returnScreen;
...
function activeNavTarget(screenName) {
  ...
  if (screenName === "profile") return profileReturnScreen;
  if (screenName === "partner-profile") {
    return partnerProfileReturnScreen === "result" ? "exchange" : "friends";
  }
  if (screenName === "detail") {
    if (detailReturnScreen === "profile") return profileReturnScreen;
    if (detailReturnScreen === "partner-profile") return "friends";
    return detailReturnScreen;
  }
  if (screenName === "settings" || screenName === "tutorial") return "";
  return screenName;
}

function updateHeaderNavigation(screenName) {
  const childScreen = {
    tags: { title: "タグ", backLabel: "描く" },
    profile: { title: "プロフィール", backLabel: "戻る" },
    "partner-profile": {
      title: "プロフィール",
      backLabel: partnerProfileReturnScreen === "friends" ? "相手" : "戻る"
    },
    settings: {
      title: "設定",
      backLabel: settingsReturnScreen === "profile" ? "プロフィール" : "ホーム"
    },
    detail: { title: "詳細", backLabel: "戻る" },
  }[screenName];

  const hasBackButton = Boolean(childScreen);
  headerBackButton.hidden = !hasBackButton;
  headerScreenTitle.hidden = !hasBackButton;
  brandButton.hidden = hasBackButton;
  settingsButton.hidden = hasBackButton;

  if (childScreen) {
    headerBackLabel.textContent = childScreen.backLabel;
    headerScreenTitle.textContent = childScreen.title;
    headerBackButton.setAttribute("aria-label", `${childScreen.backLabel}へ戻る`);
  }
}
...
function showScreen(name, updateHash = true) {
  ...
  if (nextName !== "settings") document.body.classList.remove("is-icon-editor-sheet-open");
  updateHeaderNavigation(nextName);
  ...
  if (nextName === "profile") renderSelfProfile();
  if (nextName === "partner-profile") renderPartnerProfile();
...
function playSaveConfetti() {
  ...
  const colors = ["#a95747", "#708266", "#c79b45", "#f2efe8", "#fffdf9"];
...
document.addEventListener("pointerdown", (event) => {
  if (isPalettePanelOpen && !drawingPanel.contains(event.target)) {
    isPalettePanelOpen = false;
    updatePalettePanelState();
  }
  if (isIconPalettePanelOpen && !iconDrawingPanel.contains(event.target)) {
    isIconPalettePanelOpen = false;
    updateIconPalettePanelState();
  }
});
...
iconEditorCloseButton.addEventListener("click", () => {
  iconPixels = state.profileIconPixels.slice();
  resetIconClearConfirmation();
  setIconEditorOpen(false);
});
...
iconEraserButton.addEventListener("click", () => {
  ...
  isIconEyedropper = false;
  isIconBucketFill = false;
  pulseToolButton(iconEraserButton);
  updateIconToolState();
  renderIconPalette();
});
iconEyedropperButton.addEventListener("click", () => {
  ...
  isIconErasing = false;
  isIconBucketFill = false;
  pulseToolButton(iconEyedropperButton);
  updateIconToolState();
  renderIconPalette();
...
drawTutorialTipNext.addEventListener("click", advanceDrawTutorialTip);
...
document.querySelector("#result-profile-button").addEventListener("click", () => openPartnerProfile(currentSample, "result"));
...
document.querySelector("#settings-button").addEventListener("click", () => {
  const origin = activeNavTarget(currentScreen);
  profileReturnScreen = ["today", "draw", "exchange", "records", "friends"].includes(origin)
    ? origin
    : "today";
  showScreen("profile");
});
...
partnerProfileBlockButton.addEventListener("click", () => {
  if (activePartnerProfile) blockSample(activePartnerProfile);
});
settingsBackButton.addEventListener("click", () => showScreen(settingsReturnScreen));
headerBackButton.addEventListener("click", () => {
  if (currentScreen === "tags") showScreen("draw");
  else if (currentScreen === "profile") showScreen(profileReturnScreen);
  else if (currentScreen === "partner-profile") showScreen(partnerProfileReturnScreen);
  else if (currentScreen === "settings") showScreen(settingsReturnScreen);
  else if (currentScreen === "detail") showScreen(detailReturnScreen);
});
...
const requestedInitialScreen = requestedScreen === "home"
  ...
  : requestedScreen === "partners"
    ? "friends"
    : requestedScreen === "partner-profile"
      ? "friends"
